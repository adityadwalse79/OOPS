#include<iostream>
#include<string>
using namespace std;

class car{
    protected:
        string brand;
        string model;
        bool isEngineOn;
        int currSpeed;
    public:
        car(string b,string m){
            this->brand=b;
            this->model=m;
            isEngineOn=false;
            currSpeed=0;
        }
        void start_engine(){
            isEngineOn=true;
            cout<<brand<<" "<<model<<" :Engine starts with a roar"<<endl;
        }
        void stop_engine(){
            isEngineOn=false;
            currSpeed=0;
            cout<<brand<<" "<<model<<" :Engine turned off"<<endl;
        }
        virtual void accelerate()=0;
        virtual void brake()=0;
        virtual ~car(){}  //destructor
    };
class manual_car : public car {
    private:
        int currGear;
    public:
        manual_car(string b,string m):car(b,m){
            currGear=0; //neutral
        }
        void shift_gear(int gear){
            currGear=gear;
            cout<<brand<<" "<<model<<" :Shifted to gear "<<currGear<<endl;
        }
        void accelerate(){
            if(!isEngineOn){
                cout<<brand<<" "<<model<<" :Engine is off! Cannot accelerate"<<endl;
                return;
            }
            currSpeed+=20;
            cout<<brand<<" "<<model<<" :Accelerating to "<<currSpeed<<"km/hr"<<endl;
        }
        void brake(){
            currSpeed-=20;
            if(currSpeed<0)currSpeed=0;
            cout<<brand<<" "<<model<<" :Braking speed is now: "<<currSpeed<<"km/hr"<<endl;
        }
    };
class electric_car : public car {
    private:
        int batteryLevel;
    public:
        electric_car(string b,string m):car(b,m){
            batteryLevel=100; //fully charged
        }
        void charge_battery(int amount){
            batteryLevel=100;
            cout<<brand<<" "<<model<<" :Battery fully charged"<<endl;
        }
        void accelerate(){
            if(!isEngineOn){
                cout<<brand<<" "<<model<<" :Engine is off! Cannot accelerate"<<endl;
                return;
            }
            if(batteryLevel<=0){
                cout<<brand<<" "<<model<<" :Battery is dead! Cannot accelerate"<<endl;
                return;
            }
            batteryLevel-=10;
            currSpeed+=15;
            cout<<brand<<" "<<model<<" :Accelerating to "<<currSpeed<<"km/hr"<<endl;
        }
        void brake(){
            currSpeed-=15;
            if(currSpeed<0)currSpeed=0;
            cout<<brand<<" "<<model<<" :Braking speed is now: "<<currSpeed<<"km/hr"<<endl;
        }
    };
int main(){
    car *mycar=new manual_car("Ford","Mustang");  //parent class ka ek pointer,child class ke object ko point kar sakta hai!
    mycar->start_engine();
    // mycar->shift_gear(1); //error: 'class car' has no member named 'shift_gear'
    mycar->accelerate();
    // mycar->shift_gear(2); //error: 'class car' has no
    mycar->accelerate();
    mycar->brake();
    mycar->stop_engine();
    cout<<endl;
    cout<<"------------------------------"<<endl;
    car *myElectricCar = new electric_car("Tesla","Model S");
    myElectricCar->start_engine();
    // myElectricCar->show_battery(); //error: 'class car' has no member named 'show_battery'
    myElectricCar->accelerate();
    myElectricCar->accelerate();
    myElectricCar->brake();
    myElectricCar->stop_engine();
    return 0;
}